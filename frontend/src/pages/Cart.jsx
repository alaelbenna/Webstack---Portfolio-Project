            ...
            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-4[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                <div className='flex items-start gap-6'>
                  {/* Make sure productData.image exists and has at least one image */}
                  <img className='w-16 sm:w-20' src={productData.image && productData.image[0]} alt={productData.name || 'Product Image'} />
                  <div>
                    <p className='text-xs sm:text-lg font-medium '>{productData.name}</p>
                    <div className='flex items-center gap-5 mt-2'>
                      <p>{currency}{productData.price}</p>
                      <p className='px-2 sm:-3 sm:py-1 border bg-slate-50'>{item.size}</p>
                    </div>
                  </div>
                </div>
                <input
